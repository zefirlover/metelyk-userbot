...
async def main():
    async with app:
        ...
        async for messages in app.get_chat_history(chat_id, 10):
            ...
            if messages.reactions == [] or messages.reactions == None or messages.reactions != [types.Reaction(count=int,chosen_order=0,emoji=reaction)]:
                await app.send_reaction(chat_id, messages.id, reaction)
            elif messages.reactions == [types.Reaction(count=int,emoji=reaction)]:
                await app.send_reaction(chat_id, messages.id, reaction)
